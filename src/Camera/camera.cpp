...
void Camera::evaluateRotate() {
  if (!forward.z() && !forward.y()) {
    ...
    rot.x() = float(toSita(vec2f(-1, 0), vec2f(forward.z(), forward.y())));
    if (forward.y() < 0) {
      rot.x() *= -1;
    }
  }
  ...
  if (!forward.x() && !forward.z()) {
    ...
    rot.y() = float(toSita(vec2f(0, -1), vec2f(forward.x(), forward.z())));
    if (forward.x() < 0) {
      rot.y() *= -1;
    }
  }
  ...
  if (!forward.x() && !forward.y()) {
    ...
    rot.z() = float(toSita(vec2f(0, 1), vec2f(forward.x(), forward.y())));
    if (forward.y() < 0) {
      rot.z() *= -1;
    }
  }
